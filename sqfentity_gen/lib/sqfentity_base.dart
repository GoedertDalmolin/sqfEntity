...
class SqfEntityModelConverter {
  ...
  SqfEntityFieldType getFieldProperties(SqfEntityField field, SqfEntityFieldType toField) {
    return toField
      ...
      ..sequencedBy = field.sequencedBy == null ? null : toSequence(field.sequencedBy!);
  }
...
''';
  ...
  String __createObjectField() {
    ...
    if (_table.primaryKeyName != null && _table.primaryKeyName!.isNotEmpty && !_table.primaryKeyName!.startsWith('_')) {
      retVal.writeln('''static TableField? _f${toCamelCase(_table.primaryKeyNames[0])};
...
''');
    ...
    for (SqfEntityFieldType field in _table.fields!.where((f) => !f.fieldName!.startsWith('_'))) {
      if (field is SqfEntityFieldVirtualBase || (field is SqfEntityFieldRelationshipBase && field.relationType == RelationType.MANY_TO_MANY && _table.relationType != RelationType.MANY_TO_MANY)) {
        continue;
      }
      retVal.writeln('''static TableField? _f${field.fieldName!.substring(0, 1).toUpperCase() + field.fieldName!.substring(1)};
...
''';
  ...
  String __gotoEditgetByIdParameters(SqfEntityTableBase _table) {
    ...
    for (int i = 1; i < _table.primaryKeyNames.length; i++) {
      retVal.write(', obj[\'${_table.primaryKeyNames[i]}\']${table.primaryKeyTypes[i].toLowerCase() == 'string' ? '.toString()' : ' as ${table.primaryKeyTypes[i]}'}');
    }
...
class SqfEntityObjectBuilder {
  ...
  String get _createObjectRelationsPreLoad => __createObjectRelationsPreLoad(_table);
  String get _toOnetoOneSaveCode => __toOnetoOneSaveCode(_table);
  String get _toOnetoOneSaveAsCode => __toOnetoOneSaveAsCode(_table);
  String get _hiddenMethod => _table.relationType == RelationType.ONE_TO_ONE ? '_' : '';
  ...
  String __createProperties() {
    ...
    if (_table.primaryKeyName != null && _table.primaryKeyName!.isNotEmpty) {
      _retVal.writeln('${_table.primaryKeyType == PrimaryKeyType.text ? 'String' : 'int'}? ${_table.primaryKeyNames[0]};');
    }
    for (final field in _table.fields!) {
      if (field is SqfEntityFieldRelationshipBase) {
        if (field.relationType == RelationType.MANY_TO_MANY && _table.relationType != RelationType.MANY_TO_MANY) {
          continue;
        }
      }
    ...
    _retVal..writeln(_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental || _table.primaryKeyName == null || _table.primaryKeyName!.isEmpty ? 'bool? isSaved;' : '');
...
//print('__createBaseConstructure for ${_table.tableName}');
    if (_table.primaryKeyName != null && _table.primaryKeyName!.isNotEmpty && _table.relationType != RelationType.ONE_TO_ONE) {
      _retVal
        ..write(', this.')
        ..write(_table.primaryKeyName);
    }

    for (final field in _table.fields!.where((f) => f.fieldName != null && !f.fieldName!.startsWith('_'))) {
      if (field is SqfEntityFieldVirtualBase || (field is SqfEntityFieldRelationshipBase && field.relationType == RelationType.MANY_TO_MANY && _table.relationType != RelationType.MANY_TO_MANY)) {
        continue;
      }
  ...
  String __createConstructureArgs(bool withId) {
    ...
    if (_table.primaryKeyName != null &&
        ...
        (withId || _table.primaryKeyType != PrimaryKeyType.integer_auto_incremental)) {
      _retVal.write(',?');
    ...
    for (var i = 0; i < _table.fields!.length; i++) {
      ...
      if (_table.fields![i] is SqfEntityFieldRelationshipBase) {
        final SqfEntityFieldRelationshipBase field = _table.fields![i] as SqfEntityFieldRelationshipBase;
        if ((field.relationType == RelationType.MANY_TO_MANY && _table.relationType != RelationType.MANY_TO_MANY)
    ...
    if (_table.useSoftDeleting != null && _table.useSoftDeleting! && _table.relationType != RelationType.MANY_TO_MANY) {
      _retVal.write(',?');
    }

    return _retVal.toString().length > 2 ? _retVal.toString().substring(1) : _retVal.toString();
  }

  String __toMapString() {
    final _retVal = StringBuffer();
    if (_table.relationType != RelationType.ONE_TO_ONE && _table.primaryKeyName != null && _table.primaryKeyName!.isNotEmpty) {
      _retVal.writeln('map[\'${_table.primaryKeyNames[0]}\'] = ${_table.primaryKeyNames[0]};');
    }
    for (var field in _table.fields!) {
      ...
      if (field is SqfEntityFieldRelationshipBase && field.relationType == RelationType.ONE_TO_MANY) {
        _retVal.writeln('if (${field.fieldName} != null) {map[\'${field.fieldName}\'] = forView ? pl${field.relationshipName} == null ? ${field.fieldName} : pl${field.relationshipName}!.${field.formDropDownTextField} : ${field.fieldName};} else if (${field.fieldName} != null || !forView){map[\'${field.fieldName}\'] = null;}');
      } else {
        _retVal.writeln('    ${field.toMapString()}');
      }
    }
    if (_table.useSoftDeleting != null && _table.useSoftDeleting!) {
      _retVal.writeln('  if (isDeleted != null) {map[\'isDeleted\'] = forQuery ? (isDeleted! ? 1 : 0):isDeleted;}');
    }
    return _retVal.toString();
  }
  ...
  String __toFromMapString() {
    ...
    if (_table.useSoftDeleting != null && _table.useSoftDeleting!) {
      _retVal.writeln('  isDeleted = o[\'isDeleted\'] != null ? o[\'isDeleted\'] == 1 || o[\'isDeleted\'] == true : null;');
    }
    _retVal
      //..writeln(__createObjectCollectionsFromMap().toString()) // Not tested yet
      ..writeln(__createObjectRelationsFromMap().toString());
    if (_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental || _table.primaryKeyName == null || _table.primaryKeyName!.isEmpty) {
  ...
  String __createDefaultValues() {
    ...
    final _retVal = StringBuffer(_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental || _table.primaryKeyName == null || _table.primaryKeyName!.isEmpty ? 'isSaved=false;' : '');
    for (final field in _table.fields!.where((f) => f.defaultValue != null)) {
      switch (field.dbType) {
        case DbType.time:
          field.defaultValue = 'tryParseTime(${field.defaultValue.replaceAll('\'', '\\\'').toString()})';
        ...
        case DbType.text:
          ...
          break;
        case DbType.bool:
          field.defaultValue = '${field.defaultValue.toString() == '1' || field.defaultValue.toString() == 'true' ? 'true' : 'false'}';
          break;
        default:
      }
      _retVal.writeln(' ${field.fieldName}=${field.fieldName} ?? ${field.defaultValue};');
    }
    if (_table.useSoftDeleting != null && _table.useSoftDeleting!) {
...
//print('end __createDefaultValues for ${_table.tableName}');
  ...
  String __createObjectRelations() {
    ...
    for (final field in _table.fields!.whereType<SqfEntityFieldRelationshipBase>()) {
      ...
      if (field.relationType == RelationType.ONE_TO_MANY) {
        ...
        retVal += '''/// to load parent of items to this field, use preload parameter ex: toList(preload:true) or toSingle(preload:true) or getById(preload:true)
  ...
  String __createObjectCollections() {
    ...
    for (var tableCollection in _table.collections!) {
      ...
      if (_table.relationType == RelationType.MANY_TO_MANY && tableCollection.childTable.relationType == RelationType.MANY_TO_MANY) {
        continue;
      }
      ...
      if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE) {
      ...
      final StringBuffer filterExpression = buildFilterExpression(tableCollection);
      //print('after buildFilterExpression');
      String childTableFieldTablePrimaryKeyName = tableCollection.childTableField.table == null ? _table.primaryKeyNames[0] : tableCollection.childTableField.table!.primaryKeyNames[0];

      if (tableCollection.relationType == RelationType.MANY_TO_MANY) {
        //print(' before set childTablePrimaryKeyName');
        final String? childTablePrimaryKeyName = tableCollection.childTableField.manyToManyTable!.fields!.whereType<SqfEntityFieldRelationshipBase>().firstWhere((f) => f.table!.tableName == tableCollection.childTable.tableName!).fieldName;
        //print(' before set childTableFieldTablePrimaryKeyName ');
        childTableFieldTablePrimaryKeyName = tableCollection.childTableField.manyToManyTable!.fields!.whereType<SqfEntityFieldRelationshipBase>().firstWhere((f) => f.table!.tableName == tableCollection.childTableField.table!.tableName!).fieldName!;

        retVal.writeln('''///(RelationType.MANY_TO_MANY) (${tableCollection.childTableField.manyToManyTable!.tableName}) to load children of items to this field, use preload parameter. Ex: toList(preload:true) or toSingle(preload:true) or getById(preload:true)
            /// You can also specify this object into certain preload fields!. Ex: toList(preload:true, preloadFields:['pl$funcName', 'plField2'..]) or so on..
        ...
        retVal.writeln('''/// to load children of items to this field, use preload parameter. Ex: toList(preload:true) or toSingle(preload:true) or getById(preload:true)
            /// You can also specify this object into certain preload fields!. Ex: toList(preload:true, preloadFields:['pl$funcName', 'plField2'..]) or so on..
            List<${tableCollection.childTable.modelName}>? pl$funcName;
  ...
  StringBuffer buildFilterExpression(TableCollectionBase tableCollection) {
    ...
    if (tableCollection.childTableField.table == null || tableCollection.childTableField.table == _table) {
      ...
      for (int i = 0; i < tableCollection.childTableField.relationshipFields.length; i++) {
        ...
        filterExpression.write('.${tableCollection.childTableField.relationshipFields[i].fieldName}.equals(${_table.primaryKeyNames[i]}).and');
      }
    } else {
      ...
      for (int i = 0; i < tableCollection.childTableField.relationshipFields.length; i++) {
        ...
        filterExpression.write('.${tableCollection.childTableField.relationshipFields[i].fieldName}.equals(${tableCollection.childTableField.table!.primaryKeyNames[i]}).and');
      }
    }
  ...
  String __createObjectCollectionsToMap() {
    ...
    for (var tableCollection in _table.collections!) {
      ...
      if (tableCollection.childTableField.table != null) {
        if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE) {
          final relationName = tableCollection.childTable.modelName!.toLowerCase();
          retVal.writeln('''if (!forQuery) {map['$relationName'] = await $relationName.toMapWithChildren();}''');
        } else {
          retVal.writeln('''if (!forQuery) {map['$funcName'] = await get$funcName()!.toMapList();}''');
        }
      }
    }
  ...
  String __createObjectRelationsFromMap() {
    ...
    for (final field in _table.fields!.whereType<SqfEntityFieldRelationshipBase>()) {
      if (field.relationType == RelationType.ONE_TO_MANY) {
        ...
        final String mapName = field.fieldName!.toLowerCase() == modelName.toLowerCase() ? 'pl$modelName' : tocamelCase(modelName);
        String funcName = modelName;
        if (relations.contains(funcName)) {
          funcName += 'By${toCamelCase(field.fieldName!)}';
          if (relations.contains(funcName)) {
        ...
        retVal.writeln('pl$funcName = o[\'$mapName\'] != null ? $modelName.fromMap(o[\'$mapName\'] as Map<String, dynamic>) : null;');
        relations.add(funcName);
      }
    }
    if (retVal.isNotEmpty) {
  ...
  String __deleteMethodSingle() {
    ...
    for (var tableCollection in _table.collections!) {
      ...
      if (tableCollection.childTableField.relationType == RelationType.MANY_TO_MANY) {
        continue;
      }
  ...
  String __saveMethod() {
    ...
    for (var sequencedField in _table.fields!.where((i) => i.sequencedBy != null)) {
      seq.writeln('${sequencedField.fieldName} = await ${sequencedField.sequencedBy!.modelName}().nextVal();');
    }
    final retVal = StringBuffer();
    ...
    Future<int?> ${_hiddenMethod}saveOrThrow({bool ignoreBatch = true}) async {
      ...
      if (_table.relationType != RelationType.ONE_TO_ONE && _table.primaryKeyName != null && _table.primaryKeyName!.isNotEmpty && _table.primaryKeyType == PrimaryKeyType.integer_auto_incremental) {
        retVal.writeln('''/// saveAs ${_table.modelName}. Returns a new Primary Key value of ${_table.modelName}
...
String __createObjectRelationsPreLoad(SqfEntityTableBase _table) {
  ...
  for (final field in _table.fields!.whereType<SqfEntityFieldRelationshipBase>()) {
    if (field.relationType == RelationType.ONE_TO_MANY) {
      ...
      retVal.writeln('if ((preloadFields == null || loadParents || preloadFields.contains(\'pl$funcName\'))) {obj.pl$funcName = obj.pl$funcName ?? await obj.get$funcName(loadParents: loadParents);}');
      relations.add(funcName);
    }
  }
...
String __getByIdParameters(SqfEntityTableBase _table, bool withTypes) {
  ...
  final retVal = StringBuffer()..write('${(withTypes ? '${_table.primaryKeyTypes[0]}?' : '')} ${_table.primaryKeyNames[0]}');
  for (int i = 1; i < _table.primaryKeyNames.length; i++) {
    retVal.write(', ${(withTypes ? '${_table.primaryKeyTypes[i]}?' : '')} ${_table.primaryKeyNames[i]}');
  }
  return retVal.toString();
}
...
String __toOnetoOneCollections(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections!) {
    ...
    final childTableFieldTablePrimaryKeyName = tableCollection.childTableField.table == null ? _table.primaryKeyNames[0] : tableCollection.childTableField.table!.primaryKeyNames[0];
    if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE) {
...
String __toOnetoManyCollections(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections!) {
    ...
    if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE || tableCollection.childTable.relationType == RelationType.MANY_TO_MANY) {
      continue;
    }
...
String __toOnetoOneSaveCode(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections!) {
    if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE) {
      ...
      retVal.writeln('''_$objName?.${tableCollection.childTableField.fieldName} = $childTableFieldTablePrimaryKeyName;
...
String __toOnetoOneSaveAsCode(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections!) {
    if (tableCollection.childTableField.relationType == RelationType.ONE_TO_ONE) {
      final objName = tableCollection.childTable.modelName!.toLowerCase();
      retVal.writeln('$objName.${tableCollection.childTable.primaryKeyNames[0]} = null;');
    }
  }
...
return _isDeleted = _setField(_isDeleted, 'isDeleted', DbType.bool);
    ...
    for (var tableCollection in _table.collections!) {
      switch (tableCollection.childTableField.deleteRule) {
        ...
        case DeleteRule.CASCADE:
          if (tableCollection.childTable.useSoftDeleting!) {
            retVal
              ..writeln('// Recover sub records where in (${tableCollection.childTable.modelName}) according to ${tableCollection.childTableField.deleteRule.toString()}')
              ..writeln(_buildWhereStrForList(tableCollection))
              ..writeln('.update({\'isDeleted\': 0}); ')
              ..writeln('if (!res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName}.success) {return res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName};}');
          }
          break;
        case DeleteRule.SET_DEFAULT_VALUE:
          // IN THIS CASE YOU CAN NOT RELATE SUB ROWS TO OLD PARENT ROW
  ...
  String __deleteMethodList() {
    ...
    for (var tableCollection in _table.collections!) {
      ...
      switch (tableCollection.childTableField.deleteRule) {
        case DeleteRule.SET_NULL:
          // IN THIS CASE YOU CAN NOT RECOVER CHILD ROWS
          retVal
            ..writeln('// UPDATE sub records where in (${tableCollection.childTable.modelName}) according to ${tableCollection.childTableField.deleteRule.toString()}')
            ..writeln(_buildWhereStrForList(tableCollection))
            ..writeln('.update({${_buildUpdateParamForList(tableCollection, DeleteRule.SET_NULL)}}); ')
            ..writeln('if (!res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName}.success) {return res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName};}');
          break;
        case DeleteRule.NO_ACTION:
          retVal.writeln('''// Check sub records where in (${tableCollection.childTable.modelName}) according to ${tableCollection.childTableField.deleteRule.toString()}
    ...
    errorMessage: 
        ...
        case DeleteRule.CASCADE:
          retVal
            ..writeln('// Delete sub records where in (${tableCollection.childTable.modelName}) according to ${tableCollection.childTableField.deleteRule.toString()}')
            ..writeln(_buildWhereStrForList(tableCollection))
        ...
        case DeleteRule.SET_DEFAULT_VALUE:
          ...
          retVal
            ..writeln('// UPDATE sub records where in (${tableCollection.childTable.modelName}) according to ${tableCollection.childTableField.deleteRule.toString()}')
            ..writeln(_buildWhereStrForList(tableCollection))
            ..writeln('.update({${_buildUpdateParamForList(tableCollection, DeleteRule.SET_DEFAULT_VALUE)}}); ')
            ..writeln('if (!res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName}.success) {return res${tableCollection.childTable.modelName}BY${tableCollection.childTableField.fieldName};}');
  ...
  String _buildWhereStrForList(TableCollectionBase tableCollection) {
    ...
    if (tableCollection.childTableField.table!.primaryKeyNames.length > 1) {
      final idList = '${tocamelCase(tableCollection.childTable.tableName!)}By${tableCollection.childTableField.fieldName}idList'.replaceAll('_', '');
      _retVal.writeln('final $idList = await toListPrimaryKey(false);');
      String queryIn = '${tableCollection.childTableField.relationshipFields[0].fieldName}=\${e.${tableCollection.childTableField.table!.primaryKeyNames[0]}}';
      for (int i = 1; i < tableCollection.childTableField.table!.primaryKeyNames.length; i++) {
        queryIn += ' AND ${tableCollection.childTableField.relationshipFields[i].fieldName}=\${e.${tableCollection.childTableField.table!.primaryKeyNames[i]}}';
      }
      _retVal.writeln('''
...
String _buildUpdateParamForList(TableCollectionBase tableCollection, DeleteRule rule) {
  final StringBuffer _retVal = StringBuffer()..write('\'${tableCollection.childTableField.relationshipFields[0].fieldName}\':${getUpdateValue(tableCollection.childTableField.relationshipFields[0], rule)}');
  for (int i = 1; i < tableCollection.childTableField.table!.primaryKeyNames.length; i++) {
    _retVal
      ..write(',')
      ..write('\'${tableCollection.childTableField.relationshipFields[i].fieldName}\':${getUpdateValue(tableCollection.childTableField.relationshipFields[i], rule)}');
  }
  return _retVal.toString();
}
...
class QueryParams {
  QueryParams({this.selectColumns, this.whereString, this.whereArguments, this.orderBy, this.groupBy, this.having, this.distinct, this.limit, this.offset});
...
class FilterBase extends FluentBase {
  ...
  ConjunctionBase equals(dynamic pValue) {
    param.expression = '=';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.EQuals, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.NotEQuals, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }

  ConjunctionBase equalsOrNull(dynamic pValue) {
    param.expression = '=';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.EQualsOrNull, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.NotEQualsOrNull, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase isNull() {
    filterBuilder._addedBlocks = setCriteria(0, filterBuilder.parameters, param, SqlSyntax.IsNULL.replaceAll(SqlSyntax.notKeyword, _waitingNot), filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase contains(dynamic pValue) {
    if (pValue != null) {
      filterBuilder._addedBlocks = setCriteria('%${pValue.toString()}%', filterBuilder.parameters, param, SqlSyntax.Contains.replaceAll(SqlSyntax.notKeyword, _waitingNot), filterBuilder._addedBlocks);
      _waitingNot = '';
      filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    }
    return filterBuilder;
  ...
  ConjunctionBase between(dynamic pFirst, dynamic pLast) {
    if (pFirst != null && pLast != null) {
      filterBuilder._addedBlocks = setCriteria(pFirst, filterBuilder.parameters, param, SqlSyntax.Between.replaceAll(SqlSyntax.notKeyword, _waitingNot), filterBuilder._addedBlocks, pLast);
    } else if (pFirst != null) {
      if (_waitingNot != '') {
        filterBuilder._addedBlocks = setCriteria(pFirst, filterBuilder.parameters, param, SqlSyntax.LessThan, filterBuilder._addedBlocks);
      } else {
        filterBuilder._addedBlocks = setCriteria(pFirst, filterBuilder.parameters, param, SqlSyntax.GreaterThanOrEquals, filterBuilder._addedBlocks);
      }
    } else if (pLast != null) {
      if (_waitingNot != '') {
        filterBuilder._addedBlocks = setCriteria(pLast, filterBuilder.parameters, param, SqlSyntax.GreaterThan, filterBuilder._addedBlocks);
      } else {
        filterBuilder._addedBlocks = setCriteria(pLast, filterBuilder.parameters, param, SqlSyntax.LessThanOrEquals, filterBuilder._addedBlocks);
      }
    }
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
  ...
  ConjunctionBase greaterThan(dynamic pValue) {
    param.expression = '>';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.GreaterThan, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.LessThanOrEquals, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase lessThan(dynamic pValue) {
    param.expression = '<';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.LessThan, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.GreaterThanOrEquals, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase greaterThanOrEquals(dynamic pValue) {
    param.expression = '>=';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.GreaterThanOrEquals, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.LessThan, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase lessThanOrEquals(dynamic pValue) {
    param.expression = '<=';
    filterBuilder._addedBlocks = _waitingNot == '' ? setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.LessThanOrEquals, filterBuilder._addedBlocks) : setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.GreaterThan, filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
  ...
  ConjunctionBase inValues(dynamic pValue) {
    filterBuilder._addedBlocks = setCriteria(pValue, filterBuilder.parameters, param, SqlSyntax.IN.replaceAll(SqlSyntax.notKeyword, _waitingNot), filterBuilder._addedBlocks);
    _waitingNot = '';
    filterBuilder._addedBlocks.needEndBlock![filterBuilder._blockIndex] = filterBuilder._addedBlocks.retVal;
    return filterBuilder;
  }
...
abstract class ConjunctionBase extends FluentBase {
  ...
  Future<TableBase> toSingleOrDefault();
  Future<int> toCount();
  Future<List<TableBase>> toList({bool preload = false, List<String>? preloadFields, bool loadParents = false, List<String>? loadedFields});
  Future<String> toJson();
  Future<String> toJsonWithChilds();
  Future<List<dynamic>> toMapList();
...
abstract class FluentBase {
  ...
  AddedBlocks setCriteria(dynamic pValue, List<DbParameter>? parameters, DbParameter? param, String? sqlSyntax, AddedBlocks? addedBlocks, [dynamic pValue2]) {
    bool sp = addedBlocks!.needEndBlock![addedBlocks.needEndBlock!.length - 1];
    if (pValue != null) {
      param!.whereString += parameters!.isNotEmpty ? parameters[parameters.length - 1].wOperator : '';
...
class BoolCommitResult {
  BoolCommitResult({required this.success, this.successMessage, this.errorMessage, this.commitResult});
  String? successMessage;
  String? errorMessage;
...
class SqfEntityTableBase {
  ...
  String get createListParameterForQueryWithId => __createConstructure(true, true);
  String get createListParameterForQuery => __createConstructure(true, false);
  String get createTableSQLnoForeignKeys => _createTableSQL(createForeignKeys: false);
  String _createTableSQL({bool createForeignKeys = true}) {
    ...
    for (SqfEntityFieldType field in fields!) {
      ...
      if (field is SqfEntityFieldRelationshipBase && createForeignKeys) {
        ...
        _createTableSQL.write(', FOREIGN KEY(${field.fieldName}) REFERENCES ${field.table!.tableName}(${field.table!.primaryKeyNames[0]}) ON DELETE ${field.deleteRule.toString().replaceAll('_', ' ').replaceAll('DeleteRule.', '')}');
      }
    }
    ...
    if (primaryKeyName == null || primaryKeyName!.isEmpty) {
      ...
      createTableSQL = 'Create table $tableName ($primaryKeyName $createTableSQL)';
    }
  ...
  String __createConstructure(bool isQuery, bool withId) {
    ...
    if (primaryKeyName != null && primaryKeyName!.isNotEmpty && relationType != RelationType.ONE_TO_ONE && (withId || primaryKeyType != PrimaryKeyType.integer_auto_incremental)) {
      _retVal
        ..write(',')
    ...
    for (int i = 0; i < fields!.length; i++) {
      ...
      if (fields![i] is SqfEntityFieldRelationshipBase) {
        final SqfEntityFieldRelationshipBase field = fields![i] as SqfEntityFieldRelationshipBase;
        if (field.relationType == RelationType.MANY_TO_MANY && relationType != RelationType.MANY_TO_MANY) {
          continue;
        }
      }
      ...
      if (isQuery) {
        if (fields![i].dbType == DbType.time) {
          _retVal.write(' != null ? defaultTimeFormat.format(toDateTime(${fields![i].fieldName}!)) : null');
        } else if (fields![i].dbType == DbType.date || fields![i].dbType == DbType.datetime || fields![i].dbType == DbType.datetimeUtc) {
...
abstract class SqfEntityFieldType {
  SqfEntityFieldType(this.fieldName, this.dbType, {this.defaultValue, this.minValue, this.maxValue, this.table, this.sequencedBy, this.primaryKeyIndex, this.isPrimaryKeyField, this.isNotNull, this.isUnique, this.checkCondition, this.isIndex, this.isIndexGroup, this.collate, this.formLabelText});
  String? fieldName;
  DbType? dbType;
...
class SqfEntityFieldRelationshipBase implements SqfEntityFieldType {
  ...
  SqfEntityFieldRelationshipBase init() {
    if (table != null) {
      ...
      table!.relationType = relationType == RelationType.ONE_TO_MANY ? relationType : null;
      fieldName = fieldName ?? table!.tableName! + toCamelCase(table!.primaryKeyNames[0]);
    } else {
  ...
  String toFromMapString() {
    try {
      ...
      throw Exception('ERROR when calling toFromMapString: fieldName:$fieldName dbType=${dbType.toString()}');
    }
  }
...
List<TableCollectionBase> _getCollections(SqfEntityTableBase table, SqfEntityModelBase _m) {
  ...
  for (var _table in _m.databaseTables!.where((t) => t.relationType != RelationType.MANY_TO_MANY)) {
    for (var field in _table.fields!.whereType<SqfEntityFieldRelationshipBase>()) {
      if (field.table == null && _table.tableName == table.tableName!) {
        collectionList.add(TableCollectionBase(table, field, relationType: field.relationType));
      } else if (field.table != null && field.table!.tableName == table.tableName!) {
        collectionList.add(TableCollectionBase(_table, field, relationType: field.relationType));
      }
    }
  }
...
abstract class CrudOperationsBase {
  ...
  Future<dynamic> saveOrThrow({bool ignoreBatch = true}) {
    final msg = 'saveOrThrow method requires a table which has an auto incremented primaryKey';
    throw UnimplementedError(msg);
  }
